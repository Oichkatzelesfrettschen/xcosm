...
class TrialityBigraph:
    ...
    def add_link(self, source_id: int, target_id: int, tension: float = 1.0) -> Optional[TrialityLink]:
...
def triality_preferential_attachment(
    ...
    for _ in range(n_new_links):
        ...
        candidates = [nid for nid in node_ids
                      if nid != source_id and nid not in source_neighbors]
...
def analyze_triality_distribution(bigraph: TrialityBigraph) -> Dict:
    ...
    counts = {t: 0 for t in TrialityType}
...
def run_triality_experiment(
    ...
    for round_num in range(n_attachment_rounds):
        ...
        if (round_num + 1) % 2 == 0:
            print(f"    Round {round_num + 1}: links={len(bigraph.links)}, "
    ...
    print(f"    Target ε: 0.2500")
    print(f"    Deviation: {abs(final_clustering - 0.25):.4f} ({abs(final_clustering - 0.25)/0.25*100:.1f}%)")
    ...
    print("\n  Comparison:")
    print(f"    Standard preferential attachment: ε ≈ 0.22")
