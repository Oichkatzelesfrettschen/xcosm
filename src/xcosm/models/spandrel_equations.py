...
def tensor_to_scalar_ccf(lambda_val: float = LAMBDA_CCF, E_N: float = E_OVER_N) -> float:
    """
...
def mean_delay_time(tau_max: float = 10.0, s: float = 1.1, tau_min: float = 0.04) -> float:
...
def alpha_evolution(z: float, alpha_0: float = ALPHA_0, delta_gamma: float = -0.1) -> float:
    """
...
def standardization_bias(z: float, alpha_0: float = ALPHA_0, delta_gamma: float = -0.1) -> float:
...
def laminar_flame_speed(rho: float, rho_ref: float = 1e9, s_L_ref: float = 50.0) -> float:
    """
...
def beta_color_evolution(z: float, beta_0: float = BETA_0, gamma_beta: float = -0.07) -> float:
    """
...
def host_mass_step_heaviside(M_star: float, delta: float = 0.06, M_step: float = 1e10) -> float:
    """
...
def malmquist_bias(z: float, sigma_int: float = 0.12, delta_m_lim: float = 1.0) -> float:
    """
...
def validate_all_equations() -> None:
    ...
    for name in ["BPM37093", "J0959-1828", "WDJ181058"]:
        ...
        print(
            f"   {name:>12} {info['M_WD']:>8.2f} {info['Delta_Pi']:>10.1f} {info['D_pred']:>8.2f}"
        )
