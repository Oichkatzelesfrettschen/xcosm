...
def analyze_stretch_mass_correlation(df):
    ...
    if abs(pearson_r) > 0.3:
        ...
        print("     The Spandrel 'D' signal may be the known mass-step relabeled")
    elif abs(pearson_r) > 0.1:
        print("\n  ⚠️  MODERATE CORRELATION: x₁ partially traces host mass")
        print("     The Spandrel framework captures BOTH mass effects AND additional physics")
    else:
        print("\n  ✓  WEAK/NO CORRELATION: x₁ is independent of host mass")
        print("     The Spandrel 'D' parameter captures genuinely new physics")
    ...
    print(
        f"    Low-mass hosts:  N={len(x1_low_mass)}, <x₁> = {np.mean(x1_low_mass):.3f} ± {np.std(x1_low_mass):.3f}"
    )
    print(
        f"    High-mass hosts: N={len(x1_high_mass)}, <x₁> = {np.mean(x1_high_mass):.3f} ± {np.std(x1_high_mass):.3f}"
    )
    ...
    if t_pval < 0.001:
        print("    ⚠️  Highly significant difference in stretch by host mass (>3σ)")
...
def analyze_stretch_redshift_evolution(df):
    ...
    for i in range(len(z_bins) - 1):
        ...
        if mask.sum() > 10:
            ...
            print(
                f"    z ∈ [{z_low:.1f}, {z_high:.1f}): N={mask.sum():4d}, <x₁> = {np.mean(x1_bin):+.3f} ± {np.std(x1_bin)/np.sqrt(mask.sum()):.3f}"
            )
    ...
    if len(x1_high_z) > 10:
        ...
        print("    Expected: <x₁>(z~0.05) = -0.17, <x₁>(z~0.65) = +0.34")
        print("    Expected Δx₁ ≈ 0.51")
...
def analyze_mass_corrected_residuals(df):
    ...
    if abs(r_partial) > 0.1 and p_partial < 0.05:
        ...
        print("    This supports the Spandrel framework capturing new physics")
    else:
        print("\n  ⚠️  x₁ effect on HR is fully explained by host mass")
        print("    The Spandrel 'D' may be redundant with mass-step correction")
