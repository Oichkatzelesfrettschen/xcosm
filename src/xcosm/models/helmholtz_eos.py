...
def coulomb_coupling_parameter(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> float:
    """
...
def debye_function_D3(x: float) -> float:
    ...
    if x < 0.1:
        ...
        def integrand(t):
            return t**3 / (np.exp(t) - 1) if t > 0 else 0
...
def ion_pressure_coulomb(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> float:
    """
...
def ion_energy_debye(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> float:
    """
...
def ion_cv_debye(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> float:
    """
...
def electron_degeneracy_parameter(
    rho: float, T: float, Z: float = Z_ION, A: float = A_ION
) -> float:
    """
...
def electron_pressure_thermal(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> float:
    """
    ...
    if eta > 10:
        ...
        P_th = (
            weight * P_degen * (np.pi**2 / 3.0) * (K_BOLTZMANN * T / E_F) ** 2
            + (1 - weight) * P_ideal
        )
...
def helmholtz_eos(rho: float, T: float, Z: float = Z_ION, A: float = A_ION) -> EOSResult:
    """
...
def temperature_from_pressure(
    rho: float, P_target: float, Z: float = Z_ION, A: float = A_ION, T_guess: float = 1e9
) -> float:
    """
    ...
    try:
        ...
        for _ in range(50):
            result = helmholtz_eos(rho, T, Z, A)
            dP_dT = (helmholtz_eos(rho, T * 1.01, Z, A).pressure - result.pressure) / (0.01 * T)
            if abs(dP_dT) < 1e-50:
                break
...
def demonstrate_debye_corrections():
    ...
    print(
        f"{'T [K]':>12} | {'Θ_D [K]':>10} | {'T/Θ_D':>8} | {'Γ':>8} | " f"{'η':>8} | {'γ_eff':>8}"
    )
    print("-" * 70)
    ...
    for T in [1e8, 3e8, 1e9, 3e9, 1e10]:
        ...
        print(
            f"{T:>12.2e} | {result.Theta_Debye:>10.2e} | {ratio:>8.2f} | "
    ...
    print(
        f"  P_electron:  {result.P_electron:.3e} dyn/cm² "
        f"({100*result.P_electron/result.pressure:.1f}%)"
    )
    print(
        f"  P_ion:       {result.P_ion:.3e} dyn/cm² " f"({100*result.P_ion/result.pressure:.1f}%)"
    )
    print(
        f"  P_radiation: {result.P_radiation:.3e} dyn/cm² "
        f"({100*result.P_radiation/result.pressure:.1f}%)"
    )
    print(
        f"  P_coulomb:   {result.P_coulomb:.3e} dyn/cm² "
