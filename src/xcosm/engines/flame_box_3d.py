...
def box_counting_3d(
    binary_field: np.ndarray, box_sizes: Optional[np.ndarray] = None
...
def compute_fractal_dimension_surface(Y_field: np.ndarray, threshold: float = 0.5) -> float:
    """
...
def compute_fractal_dimension_isosurface(Y_field: np.ndarray, threshold: float = 0.5) -> float:
    """
    ...
    try:
        ...
        verts, faces, normals, values = measure.marching_cubes(
            Y_field, level=threshold, spacing=(1.0, 1.0, 1.0)
        )
        ...
        for downsample in [1, 2, 4, 8]:
            ...
            try:
                v, f, _, _ = measure.marching_cubes(
                    Y_coarse, level=threshold, spacing=(downsample, downsample, downsample)
                )
...
def run_parameter_sweep(
    ...
    for i, Z in enumerate(Z_values):
        for j, rho in enumerate(rho_values):
            ...
            config = SimConfig(N=N, Z_metallicity=Z, rho_scale=rho, t_max=t_run, verbose=verbose)
...
def plot_results(results: dict, save_path: str = None):
    ...
    if len(Z) > 2:
        ...
        ax.text(
            0.05, 0.95, f"D ∝ Z^{{{coeffs[0]:.2f}}}", transform=ax.transAxes, fontsize=10, va="top"
        )
    ...
    if len(rho) > 2:
        ...
        ax.text(
            0.05, 0.95, f"D ∝ ρ^{{{coeffs[0]:.2f}}}", transform=ax.transAxes, fontsize=10, va="top"
        )
...
def plot_flame_snapshot(solver: SpectralNSSolver, save_path: str = None):
    ...
    mid = solver.N // 2
    im = ax1.imshow(
        solver.Y_scalar[:, mid, :].T, origin="lower", cmap="RdYlBu_r", extent=[0, 1, 0, 1]
    )
    ax1.set_xlabel("x")
    ax1.set_ylabel("z")
    ...
    im2 = ax2.imshow(solver.w[:, mid, :].T, origin="lower", cmap="seismic", extent=[0, 1, 0, 1])
    ax2.set_xlabel("x")
    ax2.set_ylabel("z")
...
def main():
    ...
    config = SimConfig(
        N=64, Z_metallicity=1.0, rho_scale=1.0, t_max=0.3, verbose=True  # Start small for demo
    )
    ...
    def callback(s):
        diag = s.compute_diagnostics()
        print(
            f"  Step {diag['step']:4d}: t = {diag['t']:.4f}, "
    ...
    if len(Z_values) > 2:
        ...
        if alpha < 0:
            print("  → Low metallicity increases D (as predicted)")
        else:
            print("  → Unexpected: high metallicity increases D")
    ...
    if len(rho_values) > 2:
        ...
        if beta > 0:
            print("  → Higher density increases D (stronger RT)")
        else:
            print("  → Unexpected: higher density decreases D")
