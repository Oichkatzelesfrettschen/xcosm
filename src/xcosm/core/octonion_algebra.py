...
class Jordan3O:
    ...
    def __init__(self, alpha=0.0, beta=0.0, gamma=0.0, x=None, y=None, z=None):
        # Diagonal (real)
    ...
    def jordan_product(self, other: "Jordan3O") -> "Jordan3O":
        ...
        new_alpha = self.alpha * other.alpha + (
            (self.x.conjugate() * other.x).real_part() + (self.y.conjugate() * other.y).real_part()
        )
        new_beta = self.beta * other.beta + (
            (self.x * other.x.conjugate()).real_part() + (self.z.conjugate() * other.z).real_part()
        )
        new_gamma = self.gamma * other.gamma + (
            (self.y * other.y.conjugate()).real_part() + (self.z * other.z.conjugate()).real_part()
        )

        # Off-diagonal: (A∘B)_ij = ½(A_ii + B_jj) * ... (simplified)
        new_x = self.x * (other.alpha + other.beta) * 0.5 + other.x * (self.alpha + self.beta) * 0.5
        new_y = (
            self.y * (other.alpha + other.gamma) * 0.5 + other.y * (self.alpha + self.gamma) * 0.5
        )
        new_z = self.z * (other.beta + other.gamma) * 0.5 + other.z * (self.beta + self.gamma) * 0.5
    ...
    def to_27_vector(self) -> np.ndarray:
        ...
        """
        return np.concatenate([[self.alpha, self.beta, self.gamma], self.x.c, self.y.c, self.z.c])
...
def test_jordan_algebra():
    ...
    A = Jordan3O(
        alpha=1.0, beta=2.0, gamma=3.0, x=Octonion.unit(1), y=Octonion.unit(2), z=Octonion.unit(3)
    )
