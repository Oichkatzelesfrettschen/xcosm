...
class MPSSpectralSolver:
    ...
    def _setup_stratification(self):
        ...
        if self.config.enable_stratification:
            ...
            self.rho_background_static = torch.ones(self.N, self.N, self.N, device=self.device, dtype=DTYPE)
            self.drho_dz_background = torch.zeros(self.N, self.N, self.N, device=self.device, dtype=DTYPE)
