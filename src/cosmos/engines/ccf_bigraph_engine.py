...
class CosmologicalBigraph:
    ...
    def add_node(self, node_type: NodeType, mass: float = 0.0,
                 position: np.ndarray = None, parent_id: int = None) -> Node:
    ...
    def add_link(self, source_id: int, target_id: int,
                 link_type: LinkType, tension: float = 1.0) -> Link:
...
class InflationRule(RewritingRule):
    ...
    def applicable(self, bigraph: CosmologicalBigraph) -> List[Tuple]:
        ...
        return [(n.node_id,) for n in bigraph.nodes.values()
                if n.node_type == NodeType.VACUUM]
    ...
    def apply(self, bigraph: CosmologicalBigraph, match: Tuple) -> Dict:
        ...
        child = bigraph.add_node(
            node_type=NodeType.VACUUM,
            position=new_pos,
            parent_id=parent_id
        )
...
class PreferentialAttachmentRule(RewritingRule):
    ...
    def applicable(self, bigraph: CosmologicalBigraph) -> List[Tuple]:
        ...
        matter_nodes = [n.node_id for n in bigraph.nodes.values()
                       if n.node_type in (NodeType.MATTER, NodeType.DARK_MATTER,
                                         NodeType.STRUCTURE)]
    ...
    def apply(self, bigraph: CosmologicalBigraph, match: Tuple) -> Dict:
        ...
        weight = (source_deg ** self.params.alpha_attachment +
                  target_deg ** self.params.alpha_attachment)
        ...
        bigraph.add_link(source_id, target_id, LinkType.CAUSAL,
                        tension=self.params.epsilon_tension)
