...
class AutomorphismGroup:
    ...
    def is_automorphism(self, perm: Tuple[int, ...]) -> bool:
        ...
        perm_map = {self.motif.nodes[i]: self.motif.nodes[perm[i]]
                    for i in range(self.n)}

        for u, v in self.motif.place_edges:
            mapped_edge = (perm_map.get(u, u), perm_map.get(v, v))
            if mapped_edge not in self.motif.place_edges and \
               (mapped_edge[1], mapped_edge[0]) not in self.motif.place_edges:
                return False
...
class U1Verification:
    ...
    def is_observable_invariant(self, amplitudes: List[complex],
                                theta: float) -> bool:
...
class SU2Verification:
    ...
    def verify_su2_properties(self) -> Dict:
        ...
        for _ in range(10):
            ...
            product = U1 @ U2
            if not (np.isclose(np.linalg.det(product), 1.0) and
                    np.allclose(product @ product.conj().T, np.eye(2))):
                closure_valid = False
        ...
        for i in range(3):
            for j in range(3):
                ...
                expected = 1j * sum(
                    self._structure_constant(i, j, k) * self.generators[k]
                    for k in range(3)
                )
        ...
        return {
            ...
            "is_valid_su2": all([unitarity_valid, det_valid,
                                 closure_valid, lie_algebra_valid])
        }
...
class SU3Verification:
    ...
    def verify_su3_properties(self) -> Dict:
        ...
        for _ in range(10):
            ...
            product = U1 @ U2
            if not (np.isclose(np.linalg.det(product), 1.0, atol=1e-10) and
                    np.allclose(product @ product.conj().T, np.eye(3), atol=1e-10)):
                closure_valid = False
        ...
        return {
            ...
            "is_valid_su3": all([unitarity_valid, det_valid,
                                 closure_valid, traceless_valid])
        }
...
class StandardModelVerification:
    ...
    def verify_anomaly_cancellation(self) -> Dict:
        ...
        y_sum = sum(l["Y"] * l["mult"] for l in leptons) + \
                sum(q["Y"] * q["mult"] for q in quarks)

        y3_sum = sum(l["Y"]**3 * l["mult"] for l in leptons) + \
                 sum(q["Y"]**3 * q["mult"] for q in quarks)
...
def run_gauge_emergence_verification():
    ...
    for name, data in hypercharges.items():
        ...
        print(f"    {name:6s}: Y = {data['Y']:+.3f} "
              f"(computed: {data['computed_Y']:+.3f}) [{status}]")

    print("\n  Anomaly cancellation:")
    anomaly = sm_verifier.verify_anomaly_cancellation()
    print(f"    sum(Y):   {anomaly['sum_Y']:.6f} "
          f"(cancelled: {anomaly['linear_cancelled']})")
    print(f"    sum(Y^3): {anomaly['sum_Y3']:.6f} "
          f"(cancelled: {anomaly['cubic_cancelled']})")
    ...
    all_valid = (
        u1_results['is_valid_u1'] and
        su2_results['is_valid_su2'] and
        su3_results['is_valid_su3'] and
        anomaly['anomaly_free']
    )
