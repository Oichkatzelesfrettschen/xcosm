    ...
    for weight in weights:
        ...
        for seed in range(n_seeds):
            history, nodes = run_triality_equilibrium(
                n_final=n_final, m_edges=m_edges, triality_weight=weight, seed=seed * 17 + 42
            )
            if history:
...
def detailed_analysis(best_weight: float, n_final: int = 3000, seed: int = 42):
    ...
    history, nodes = run_triality_equilibrium(
        n_final=n_final, m_edges=3, triality_weight=best_weight, seed=seed, record_interval=100
    )
