...
class Prediction:
    ...
    def tension(self) -> float:
        ...
        combined_sigma = np.sqrt(
            self.predicted_uncertainty**2 + self.observed_uncertainty**2
        )
        if combined_sigma == 0:
            return 0.0
...
def analyze_predictions(predictions: list):
    ...
    for pred in predictions:
        ...
        print(f"{pred.symbol:<25} {pred_str:>12} {obs_str:>12} {tension_str:>8} {status_marker} {pred.status:<10}")
    ...
    print(f"  COMPATIBLE (1-2σ): {statuses['COMPATIBLE']} ({100*statuses['COMPATIBLE']/total:.0f}%)")
