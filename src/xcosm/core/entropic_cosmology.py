...
def comoving_distance(z: float, Omega_m: float, xi: float, model: str = "entropic") -> float:
    """
    ...
    integrand = integrand_lcdm if model == "LCDM" else integrand_entropic
    result, _ = quad(integrand, 0, z, limit=100)
    return result


def luminosity_distance(
    z: float, Omega_m: float, xi: float, H0: float = 70.0, model: str = "entropic"
) -> float:
    """
...
def distance_modulus_array(
    z_array: np.ndarray, Omega_m: float, xi: float, H0: float = 70.0, model: str = "entropic"
) -> np.ndarray:
...
def generate_synthetic_pantheon(
    n_sne: int = 100,
...
def compute_evidence_ratio(
    z_data: np.ndarray, mu_data: np.ndarray, sigma_data: np.ndarray, n_samples: int = 10000
) -> float:
    """
    ...
    result = run_mcmc_simple(
        z_data, mu_data, sigma_data, model="entropic", n_steps=2000, n_walkers=16, burn_in=500
    )
...
def test_synthetic_fit():
    ...
    print(
        f"    Best Ω_m = {result_ent['best_Omega_m']:.3f}, ξ = {result_ent['best_xi']:.3f}, χ² = {result_ent['best_chi2']:.1f}"
    )
...
def demo_entropic_vs_lcdm():
    ...
    for i in [0, 10, 25, 50, 75, 99]:
        ...
        print(
            f"  {z:.2f}  | {E_lcdm[i]:.4f}  | {E_ent_010[i]:.4f}    | {E_ent_025[i]:.4f}    | {delta:+.2f}%"
        )
