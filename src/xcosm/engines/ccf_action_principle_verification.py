...
class ActionFunctional:
    ...
    def action_variation(
        self, state: BigraphState, operation: str, target: Optional[int] = None
    ) -> float:
    ...
    def _apply_operation(
        self, state: BigraphState, operation: str, target: Optional[int]
    ) -> BigraphState:
...
class UniquenessVerifier:
    ...
    def __init__(self, target_ns: float = 0.966, target_s8: float = 0.78, target_w0: float = -0.83):
    ...
    def simulate_evolution(
        self, lmbda: float, alpha: float, epsilon: float, steps: int = 100, seed: int = 42
    ) -> Dict[str, float]:
        ...
        rules = [InflationRule(lmbda), AttachmentRule(alpha), ExpansionRule(epsilon)]
    ...
    def find_optimal_parameters(self) -> Tuple[np.ndarray, float]:
        ...
        result = differential_evolution(
            self.constraint_violation, bounds, seed=42, maxiter=100, tol=1e-4, polish=True
        )
    ...
    def test_alternative_rules(self, num_alternatives: int = 100) -> Dict:
        ...
        for i in range(num_alternatives):
            alt_params = np.array(
                [rng.uniform(0.001, 0.1), rng.uniform(0.1, 2.0), rng.uniform(0.01, 1.0)]
            )
...
class StationarityChecker:
    ...
    def check_attachment_stationarity(self, alpha: float = 0.85) -> Dict:
        ...
        for i in range(20):
            ...
            delta_s = self.action.action_variation(state, "add_edge", (source, target))
            variations.append(delta_s)
...
def run_action_principle_verification():
    ...
    all_stationary = (
        inflation_result["is_stationary"]
        and attachment_result["is_stationary"]
        and expansion_result["is_stationary"]
    )

    is_unique = alt_results["alternatives_better"] == 0

    params_close = (
        abs(optimal_params[0] - 0.003) < 0.01
        and abs(optimal_params[1] - 0.85) < 0.2
        and abs(optimal_params[2] - 0.25) < 0.1
    )
    ...
    return {
        ...
        "continuum": {"convergence": convergence, "valid": continuum_valid},
