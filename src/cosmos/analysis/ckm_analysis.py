...
def ckm_matrix(theta_12: float, theta_23: float, theta_13: float,
...
def extract_angles(V: np.ndarray) -> Dict[str, float]:
    ...
    denom = np.cos(theta_12) * np.sin(theta_12) * \
            np.cos(theta_23) * np.sin(theta_23) * \
            np.cos(theta_13)**2 * np.sin(theta_13)
...
def run_ckm_rg(theta_12_gut: float, theta_23_gut: float, theta_13_gut: float,
               delta_gut: float = 68.0,
               mu_gut: float = 2e16, mu_low: float = 2.0) -> Dict[str, float]:
    """
    ...
    with warnings.catch_warnings():
        ...
        sol = solve_ivp(ckm_beta, t_span, y0, method='RK45',
...
def fit_j3o_parameters():
    ...
    def chi_squared(params):
        ...
        # Compute χ²
        chi2 = 0
        chi2 += ((result['theta_12'] - CKM_EXPERIMENTAL['theta_12'])
                 / CKM_EXPERIMENTAL['sigma_theta_12'])**2
        chi2 += ((result['theta_23'] - CKM_EXPERIMENTAL['theta_23'])
                 / CKM_EXPERIMENTAL['sigma_theta_23'])**2
        chi2 += ((result['theta_13'] - CKM_EXPERIMENTAL['theta_13'])
                 / CKM_EXPERIMENTAL['sigma_theta_13'])**2
...
def analyze_ckm():
    ...
    rg_result = run_ckm_rg(
        j3o_pred['theta_12_gut'],
        j3o_pred['theta_23_gut'],
        j3o_pred['theta_13_gut']
    )
    ...
    deviations = {
        'theta_12': (rg_result['theta_12'] - CKM_EXPERIMENTAL['theta_12'])
                    / CKM_EXPERIMENTAL['theta_12'] * 100,
        'theta_23': (rg_result['theta_23'] - CKM_EXPERIMENTAL['theta_23'])
                    / CKM_EXPERIMENTAL['theta_23'] * 100,
        'theta_13': (rg_result['theta_13'] - CKM_EXPERIMENTAL['theta_13'])
                    / CKM_EXPERIMENTAL['theta_13'] * 100,
    }

    print(f"  θ₁₂: {rg_result['theta_12']:.2f}° vs {CKM_EXPERIMENTAL['theta_12']:.2f}°"
          f"  (deviation: {deviations['theta_12']:+.1f}%)")
    print(f"  θ₂₃: {rg_result['theta_23']:.2f}° vs {CKM_EXPERIMENTAL['theta_23']:.2f}°"
          f"  (deviation: {deviations['theta_23']:+.1f}%)")
    print(f"  θ₁₃: {rg_result['theta_13']:.3f}° vs {CKM_EXPERIMENTAL['theta_13']:.3f}°"
