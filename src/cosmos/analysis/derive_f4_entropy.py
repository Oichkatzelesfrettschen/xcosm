...
from core.octonion_algebra import Octonion, Jordan3O
...
def numerical_gradient_entropy(element: Jordan3O, epsilon: float = 1e-6) -> np.ndarray:
    ...
    # Perturb α
    perturbed = Jordan3O(
        element.alpha + epsilon, element.beta, element.gamma,
    ...
    # Perturb β
    perturbed = Jordan3O(
        element.alpha, element.beta + epsilon, element.gamma,
        element.x, element.y, element.z
    )
    ...
    # Perturb γ
    perturbed = Jordan3O(
        element.alpha, element.beta, element.gamma + epsilon,
        element.x, element.y, element.z
    )
...
def verify_inverse_law():
    ...
    print(f"    ∇S × X = {product}")
    print(f"    Should be ≈ [1, 1, 1]: {'YES ✓' if np.allclose(product, 1.0, rtol=0.01) else 'NO ✗'}")
...
def entropy_driven_expansion():
    ...
    print(f"    H decreases as 1/a: decelerating expansion ✓")
...
def derive_epsilon_from_entropy():
    ...
    print(f"    Freudenthal coefficient: 1/4 = 0.2500 (exact)")
    print(f"    ε_tension = 1/4 = 0.2500 (CCF parameter)")
