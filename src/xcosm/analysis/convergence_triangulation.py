...
def run_metallicity_sweep(N: int, n_steps: int = 400) -> Dict:
    ...
    for Z in METALLICITY_VALUES:
        ...
        results[Z] = {"D": D_final, "D_samples": D_samples, "elapsed": elapsed}
...
def fit_convergence(N_values: np.ndarray, beta_values: np.ndarray) -> Dict:
    ...
    try:
        ...
        popt, pcov = curve_fit(
            power_law_model, N_values, beta_values, p0=p0, bounds=bounds, maxfev=10000
        )

        beta_inf, A, p = popt
        perr = np.sqrt(np.diag(pcov))

        return {"beta_inf": beta_inf, "A": A, "p": p, "beta_inf_err": perr[0], "success": True}
...
def main():
    ...
    for N in hero_resolutions:
        if fit_result["success"]:
            beta_pred = power_law_model(N, fit_result["beta_inf"], fit_result["A"], fit_result["p"])
        else:
    ...
    # TODO: Save comprehensive results once variables are computed
