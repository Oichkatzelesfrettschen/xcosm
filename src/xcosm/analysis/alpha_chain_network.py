...
def Q_value(reactants: list, products: list) -> float:
    ...
    Returns Q in erg.
    """
    Q_MeV = sum(ISOTOPES[r].mass_excess for r in reactants) - sum(
        ISOTOPES[p].mass_excess for p in products
    )
...
def energy_generation_rate(Y: np.ndarray, rho: float, T: float) -> float:
    ...
    # Energy per gram
    eps = Q_C12_C12 * r_CC + Q_C12_O16 * r_CO + Q_O16_O16 * r_OO + Q_Si_Ni * r_Si_NSE / 2
...
def evolve_network(
    Y0: np.ndarray, rho: float, T: float, dt: float, method: str = "Radau"
    ...
    eps_avg = 0.5 * (energy_generation_rate(Y0, rho, T) + energy_generation_rate(Y_new, rho, T))
...
def initial_composition_with_metals(
    X_C: float = 0.48, X_O: float = 0.48, X_Ne: float = 0.02, X_Mg: float = 0.02
) -> np.ndarray:
    """
...
def demonstrate_network():
    ...
    for step in range(n_steps):
        ...
        if step % 20 == 0:
            print(
                f"{t_total:>10.3f} | {Y[1]:>8.4f} | {Y[2]:>8.4f} | " f"{Y[5]:>8.4f} | {Y[12]:>8.4f}"
            )
