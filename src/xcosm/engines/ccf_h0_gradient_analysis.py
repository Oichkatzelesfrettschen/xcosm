...
def ccf_h0_prediction(
    log_k: np.ndarray, h0_cmb: float = 67.4, gradient: float = 1.15, k_star: float = 0.01
) -> np.ndarray:
    """
...
def ccf_h0_from_bigraph(log_k: np.ndarray, params: CCFParameters = PARAMS) -> np.ndarray:
    """
...
def bootstrap_gradient_uncertainty(
    observations: List[H0Measurement], n_bootstrap: int = 10000, seed: int = 42
) -> Dict:
    """
...
def run_complete_analysis(save_outputs: bool = True) -> Dict:
    ...
    print("  CCF predicts: m = +1.15 km/s/Mpc/decade")
    print(f"  Observed:     m = +{result.gradient:.2f} ± {result.gradient_error:.2f}")
    print(
        f"  Agreement:    {abs(result.gradient - 1.15) / result.gradient_error:.1f}σ from prediction"
    )
