...
def inject_dz_bias(sne_data, bias_model="son2025"):
    ...
    return {**sne_data, "mu_biased": mu_biased, "delta_mu_bias": delta_mu, "bias_model": bias_model}
...
def detect_phantom_crossing(w0, wa):
    ...
    """
    if wa == 0:
        return {"phantom_today": w0 < -1, "phantom_crossing": False, "z_cross": None}
    ...
    for bias_model in bias_models:
        ...
        for i in range(n_realizations):
            ...
            if verbose and i == 0:
                print(f"\nRealization {i+1}:")
                print(
                    f"  Unbiased: w₀ = {fit_unbiased['w0']:.3f} ± {fit_unbiased['w0_err']:.3f}, "
                    f"wₐ = {fit_unbiased['wa']:.3f} ± {fit_unbiased['wa_err']:.3f}"
                )
                print(
                    f"  Biased:   w₀ = {fit_biased['w0']:.3f} ± {fit_biased['w0_err']:.3f}, "
        ...
        print("TRUE COSMOLOGY: w₀ = -1.000, wₐ = 0.000 (ΛCDM)")
        print("")
        print(
            f"UNBIASED FIT:   w₀ = {w0_unbiased_mean:.3f} ± {np.std(w0_unbiased_list):.3f}, "
            f"wₐ = {wa_unbiased_mean:.3f} ± {np.std(wa_unbiased_list):.3f}"
        )
        print(
            f"BIASED FIT:     w₀ = {w0_biased_mean:.3f} ± {np.std(w0_biased_list):.3f}, "
        ...
        print("DESI observed:    w₀ = -0.72, wₐ = -2.77")
        ...
        if w0_match and wa_match:
            print("✓ SIMULATION REPRODUCES DESI-LIKE PHANTOM CROSSING!")
        elif w0_biased_mean > -1 and wa_biased_mean < 0:
            print("⚡ PARTIAL MATCH: Direction correct (w₀ > -1, wₐ < 0)")
        else:
            print("✗ NO MATCH: Bias model does not reproduce DESI signal")
...
def analyze_bias_magnitude():
    ...
    for A_bias in bias_amplitudes:
        ...
        print(
            f"    {A_bias:.2f} mag    |   {fit['w0']:.3f}   |   {fit['wa']:.3f}   |   {crossing_str}"
        )
