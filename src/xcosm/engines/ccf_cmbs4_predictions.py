...
class CCFInflationModel:
    ...
    def tensor_power_spectrum(self, k: np.ndarray, k_pivot: float = 0.05) -> np.ndarray:
        """
        ...
        power = A_t * (k / k_pivot) ** (
            self.tensor_spectral_index + 0.5 * self.running_alpha_t * log_k_ratio
        )

        return power

    def gravitational_wave_energy_density(self, k: np.ndarray, k_pivot: float = 0.05) -> np.ndarray:
        """
...
def compare_inflation_models() -> Dict:
    ...
    models = {
        ...
        "Starobinsky": CCFInflationModel(
            lambda_slow_roll=0.004, eta_slow_roll=0.033, cos2_theta=1.0
        ),  # Single-field
        "Chaotic_m2phi2": CCFInflationModel(
            lambda_slow_roll=0.01, eta_slow_roll=0.02, cos2_theta=1.0
        ),
        "Natural": CCFInflationModel(lambda_slow_roll=0.005, eta_slow_roll=0.025, cos2_theta=0.8),
    }
    ...
    for name, model in models.items():
        ...
        print(
            f"{name:20s} {model.spectral_index:>8.4f} {model.tensor_to_scalar:>8.4f} {model.tensor_spectral_index:>8.5f} {r_consistency:>8.2f} {test_result:>10s}"
        )
...
def analyze_cmbs4_detectability():
    ...
    print("\nDetection Significance:")
    print(
        f"  r detection: {sig_r:.1f}σ ({'DETECTED' if sig_r >= 3 else 'MARGINAL' if sig_r >= 2 else 'NOT DETECTED'})"
    )
...
def plot_tensor_predictions():
    ...
    ax1.fill_between(ns_range, 0, 0.036, alpha=0.2, color="gray", label="Planck 95% CL")
    ax1.axhline(0.032, color="black", linestyle="--", alpha=0.5, label="BICEP/Keck limit")

    # CMB-S4 sensitivity
    ax1.axhline(0.001, color="orange", linestyle=":", linewidth=2, label="CMB-S4 (5σ)")
    ...
    ax2.plot(r_range, nt_standard, "k-", linewidth=2, label=r"Standard: $r = -8n_t$")
    ...
    ax2.scatter(r_ccf, -r_ccf / 8, color="blue", marker="o", s=100, label="Standard at same r")
    ...
    for year, sens, exp in zip(years, sensitivities, experiments):
        ax4.annotate(exp, xy=(year, sens), xytext=(5, 10), textcoords="offset points", fontsize=9)

    ax4.axhline(
        r_ccf, color="red", linestyle="--", linewidth=2, label=f"CCF prediction (r={r_ccf:.3f})"
    )
    ax4.fill_between([2024, 2032], r_ccf - 0.003, r_ccf + 0.003, alpha=0.3, color="red")
    ...
    plt.tight_layout()
    plt.savefig("output/plots/ccf_cmbs4_predictions.png", dpi=300, bbox_inches="tight")
    print("Figure saved to output/plots/ccf_cmbs4_predictions.png")
