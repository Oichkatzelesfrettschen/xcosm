...
try:
    from xcosm.models.helmholtz_eos import (  # noqa: F401
        EOSResult,
        helmholtz_eos,
...
class RiemannHydroSolver:
    ...
    def __init__(
        self, n_cells: int = 512, domain_size: float = 1e7, fractal_dimension: float = D_FRACTAL_REF
    ) -> None:
        """
        ...
        print(
            f"EOS mode: {'Helmholtz (Debye+Coulomb)' if self.use_helmholtz else 'Polytropic (γ=4/3)'}"
        )
    ...
    def _get_primitive_vars(
        ...
        internal_energy[internal_energy < PRESSURE_FLOOR / (GAMMA_EOS - 1.0)] = PRESSURE_FLOOR / (
            GAMMA_EOS - 1.0
        )
    ...
    def _invert_energy_for_temperature(
        self, rho: float, e_target: float, T_guess: float = 1e9
    ) -> float:
        """
    ...
    def _apply_source_term(self, state: NDArray[np.float64], dt: float) -> NDArray[np.float64]:
        ...
        source_e = (
            self.resonance_amplitude * self.burning_rate_factor * density * activation * oscillation
        )
    ...
    def _apply_advection_term(self, state: NDArray[np.float64], dt: float) -> NDArray[np.float64]:
        ...
        s_max = np.maximum(
            np.abs(velocity) + sound_speed, np.abs(np.roll(velocity, -1)) + np.roll(sound_speed, -1)
        )

        flux_interface = 0.5 * (flux + np.roll(flux, -1, axis=1)) - 0.5 * s_max * (
            np.roll(state, -1, axis=1) - state
        )
    ...
    def _update_plot(self, fig, ax1, ax2, step: int, is_final: bool = False) -> None:
...
def create_solver_for_progenitor(
    metallicity: float = 1.0, n_cells: int = 512, domain_size: float = 1e7
) -> RiemannHydroSolver:
    """
    ...
    return RiemannHydroSolver(n_cells=n_cells, domain_size=domain_size, fractal_dimension=D)
