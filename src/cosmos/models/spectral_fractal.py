...
def compare_to_archetypes() -> Dict:
    ...
    for name, obs in archetypes.items():
        ...
        try:
            D_fit = brentq(
                lambda D: Ni56_mass_from_D(D) - obs['observed_M_Ni'],
                D_MIN + 0.01, D_MAX - 0.01
            )
        except ValueError:
            D_fit = D_REF
...
def print_wang2009_validation():
    ...
    print(f"{'SN':>12} | {'v_Si obs':>8} | {'v_Si pred':>9} | {'Residual':>8} | {'Δm₁₅':>5} | {'D':>5} | {'Class':>5}")
    print("-" * 72)

    for r in validation['sample']:
        print(f"{r['name']:>12} | {r['v_Si_obs']:>8.0f} | {r['v_Si_pred']:>9.0f} | "
...
def demonstrate_spectral_spandrel():
    ...
    for D in [2.2, 2.4, 2.6, 2.73, 2.8, 2.9]:
        props = predict_spectral_properties(D)
        print(f"{D:>6.2f} | {props.M_Ni56:>8.3f} | {props.E_kin/1e51:>10.2f} | "
    ...
    for z in [0.05, 0.5, 1.0, 2.0, 2.9]:
        ...
        print(f"{z:>6.2f} | {props.D:>6.3f} | {props.M_Ni56:>8.3f} | "
    ...
    for name, data in archetypes.items():
        ...
        print(f"  M_Ni: predicted {data['M_Ni_predicted']:.3f}, observed {data['M_Ni_observed']:.2f} M☉")
        print(f"  v_Si: predicted {data['v_Si_predicted']:.0f}, observed {data['v_Si_observed']:.0f} km/s")
