...
class PartitionFunction:
    ...
    def discrete_Z(self, spacetime: DiscreteSpacetime, action_type: str = "regge") -> float:
        """
...
def continuum_partition_function(Lambda: float, G: float = 1.0, V: float = 1.0) -> float:
    """
...
def verify_continuum_limit():
    ...
    for N in N_values:
        Z_mean, Z_std = pf.monte_carlo_Z(N, n_samples=500, action_type="entropic", xi=0.15)
...
def coarse_grain_action():
    ...
    for B in block_sizes:
        N_blocks = N_fine // B
        phi_coarse = np.array([np.mean(phi_fine[i * B : (i + 1) * B]) for i in range(N_blocks)])

        # Check Gaussianity (kurtosis → 3 for Gaussian)
        kurt = np.mean(phi_coarse**4) / np.mean(phi_coarse**2) ** 2
        print(f"  B = {B:2d}: {N_blocks:3d} blocks, kurtosis = {kurt:.3f}" f" (Gaussian = 3.0)")
