...
def E_z(z: float, Omega_m: float = OMEGA_M_FIDUCIAL,
...
def luminosity_distance(z: float, Omega_m: float = OMEGA_M_FIDUCIAL,
                         w0: float = -1.0, wa: float = 0.0,
                         H0: float = H0_FIDUCIAL) -> float:
    """
    Luminosity distance to redshift z (flat universe).

    D_L(z) = (1+z) × (c/H₀) × ∫₀ᶻ dz'/E(z')
    """
    integrand = lambda zp: 1.0 / E_z(zp, Omega_m, w0, wa)
    result, _ = quad(integrand, 0, z)
    D_C = (C_LIGHT_KMS / H0) * result
    return (1 + z) * D_C


def distance_modulus_theory(z: float, Omega_m: float = OMEGA_M_FIDUCIAL,
                             w0: float = -1.0, wa: float = 0.0,
                             H0: float = H0_FIDUCIAL) -> float:
    """
...
def observed_distance_modulus_biased(z: float, true_w0: float = -1.0,
                                      true_wa: float = 0.0,
                                      include_age: bool = True) -> float:
    """
...
def fit_apparent_dark_energy(z_data: np.ndarray, mu_obs: np.ndarray,
                              sigma_mu: float = 0.15) -> Tuple[float, float]:
    """
...
def demonstrate_phantom_mimicry(z_max: float = 2.0, n_points: int = 50,
...
def validate_against_desi() -> Dict:
    ...
    print("-" * 70)
    print(f"{'w₀':<12} {comparison['spandrel_w0']:>12.3f} "
          f"{desi_w0:>8.2f} ± {desi_w0_err:.2f} "
          f"{comparison['w0_tension_sigma']:>8.1f}σ")
    print(f"{'wₐ':<12} {comparison['spandrel_wa']:>12.3f} "
...
class SpandrelEvolutionEquation:
    ...
    def D_of_z(self, z: float) -> float:
        ...
        return (self.params.D_0 +
                self.params.delta_D * (1 + z)**self.params.n)
...
if __name__ == "__main__":
    ...
    print(f"is reproduced by Spandrel bias (w₀={comparison['spandrel_w0']:.3f}, wₐ={comparison['spandrel_wa']:.3f})")
    print()
