...
def compare_models_to_observations():
    ...
    for gal in JWST_EARLY_GALAXIES:
        ...
        print("{:20s} {:>6.1f} {:>12.1f} {:>12.1f} {:>12.1f} {:>10s}".format(
            gal.name[:20], gal.redshift, gal.stellar_mass_log,
            lcdm_max, ccf_max, status))
...
def analyze_red_monsters():
    ...
    for z in [5.0, 5.5, 6.0, 6.5, 7.0]:
        ...
        print("{:>6.1f} {:>12.2f} {:>12.2f} {:>12.2f}× {:>12.1f}".format(
            z, alpha, epsilon, enhance, m_max))
...
def plot_ccf_vs_lcdm():
    ...
    ax1.fill_between(z_range, 8, lcdm_max, alpha=0.3, color='blue',
                     label='ΛCDM allowed')
    ax1.fill_between(z_range, lcdm_max, ccf_max, alpha=0.3, color='red',
                     label='CCF extension')
    ...
    for gal in JWST_EARLY_GALAXIES:
        ...
        ax1.errorbar(gal.redshift, gal.stellar_mass_log,
    ...
    ax2.plot(z_range, enhancement_z, 'r-', linewidth=2.5,
             label=r'M$_*$ enhancement factor')

    ax2.axhline(1.0, color='gray', linestyle='--', alpha=0.5)
    ax2.axvline(8.0, color='gray', linestyle=':', alpha=0.5,
                label=r'$z_\mathrm{transition}$')
    ...
    plt.tight_layout()
    plt.savefig('output/plots/ccf_jwst_comparison.png',
