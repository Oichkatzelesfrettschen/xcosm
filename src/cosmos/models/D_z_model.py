...
def validate_model():
    ...
    for i, z in enumerate(z_test):
        print(f"  z = {z:.1f}: Δm_model = {bias_model[i]:+.3f} mag, "
...
def plot_predictions():
    ...
    ax.scatter([0.05, 0.65, 2.9], [-0.17, 0.34, 2.2], color='black',
               s=100, zorder=5, label='Observations')
    ax.set_xlabel('Redshift z')
    ...
    ax = axes[1, 1]
    ax.plot(z_range, mean_metallicity_z(z_range), 'purple', linewidth=2,
            label='⟨Z⟩/Z☉')
    ax.plot(z_range, sfr_relative_z(z_range) / 10, 'orange', linewidth=2,
            label='SFR (scaled)')
    ax.set_xlabel('Redshift z')
