...
class CosmologyGPU:
    ...
    def comoving_distance_batch(
        self, z_array: torch.Tensor, Omega_m: float, xi: float, n_integration: int = 100
    ) -> torch.Tensor:
        """
    ...
    def distance_modulus_batch(
        self, z_array: torch.Tensor, Omega_m: float, xi: float, H0: float = 70.0
    ) -> torch.Tensor:
...
class ParallelMCMC:
    ...
    def run(self, p0: np.ndarray, n_steps: int = 1000, progress: bool = True) -> dict:
        """
        ...
        with mp.Pool(processes=min(N_CPU_CORES, self.n_walkers)) as pool:
            sampler = emcee.EnsembleSampler(
                self.n_walkers, self.ndim, self.log_posterior, pool=pool
            )
        ...
        print(
            f"[MCMC] Completed in {elapsed:.1f}s ({n_steps*self.n_walkers/elapsed:.0f} samples/s)"
        )
...
class QCDGPU:
    ...
    def mass_running_batch(
        self, mu_array: torch.Tensor, m_ref: float, mu_ref: float, nf: int = 5
    ) -> torch.Tensor:
...
def benchmark_gpu_vs_cpu():
    ...
    return {"gpu_time": gpu_time, "cpu_time": cpu_time, "speedup": cpu_time / gpu_time}
