...
def create_plots(df):
    ...
    ax.scatter(
        low_d["x1_stretch"], low_d["H0"], alpha=0.5, s=20, label="x₁ < 0 (LOW-D)", color="blue"
    )
    ax.scatter(
        high_d["x1_stretch"], high_d["H0"], alpha=0.5, s=20, label="x₁ > 0 (HIGH-D)", color="red"
    )
    ax.axhline(
        low_d["H0"].mean(),
        color="blue",
        linestyle="--",
        linewidth=2,
        label=f'Mean LOW-D: {low_d["H0"].mean():.2f}',
    )
    ax.axhline(
        high_d["H0"].mean(),
        color="red",
        linestyle="--",
        linewidth=2,
        label=f'Mean HIGH-D: {high_d["H0"].mean():.2f}',
    )
    ...
    ax = axes[0, 1]
    ax.scatter(
        low_d["redshift"], low_d["H0"], alpha=0.5, s=20, label="x₁ < 0 (LOW-D)", color="blue"
    )
    ax.scatter(
        high_d["redshift"], high_d["H0"], alpha=0.5, s=20, label="x₁ > 0 (HIGH-D)", color="red"
    )
...
def main():
    ...
    for z_min in sorted(df_binned["z_min"].unique()):
        ...
        if len(low_d_row) > 0:
            ...
            print(
                f"  LOW-D (x₁ < 0): N={row['n_sne']:3d}, H₀={row['mean_h0']:6.2f} ± {row['std_h0']:5.2f} km/s/Mpc"
            )

        if len(high_d_row) > 0:
            row = high_d_row.iloc[0]
            print(
                f"  HIGH-D (x₁ > 0): N={row['n_sne']:3d}, H₀={row['mean_h0']:6.2f} ± {row['std_h0']:5.2f} km/s/Mpc"
            )
