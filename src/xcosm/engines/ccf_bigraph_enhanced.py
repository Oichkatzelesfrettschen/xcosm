...
try:
    from xcosm.engines.ccf_bigraph_engine import (  # noqa: F401
        PARAMS,
        CosmologicalBigraph,
...
def estimate_spectral_dimension(
    adjacency: dict[int, set[int]], t_max: int = 10, n_walks: int = 5000, seed: int = 42
) -> dict:
    """
...
def compute_motif_ratios(adjacency: dict[int, set[int]]) -> dict:
    ...
    # Candidate ε ratios
    ratio_clustering = (
        n_triangles / (n_triangles + n_triplets) if (n_triangles + n_triplets) > 0 else 0.0
    )
...
def simulate_bao_wave(
    adjacency: dict[int, set[int]], seed_node: int, freeze_after: int = 10, n_shells: int = 20
) -> dict:
    """
...
class DualBigraph:
    ...
    def __init__(self, seed: int = 42):
        ...
        self.place_parent: dict[int, int | None] = {}
    ...
    def add_node(self, node_type: str = "vacuum", parent: int | None = None) -> int:
...
class EnhancedCosmologySimulator:
    ...
    def run_full_simulation(self, inflation_steps: int = 50, structure_steps: int = 100) -> dict:
        ...
        print("  Target: d_s → 4 for emergent 4D spacetime")
        ...
        print("  Target: clustering_ratio → 0.25?")

        # Twist (γ-like parameter)
        twist = self.bigraph.compute_twist()
        print("\n[PLACE/LINK TWIST - γ analog]")
        print(f"  τ = {twist:.4f}")
        print("  Interpretation: High τ → torsion/Immirzi effects")
...
def main():
    ...
    results = sim.run_full_simulation(inflation_steps=60, structure_steps=120)
