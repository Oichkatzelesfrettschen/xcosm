...
class RGRunning:
    ...
    def run_mass(self, m_initial: float, mu_initial: float,
...
class AEGAnalysis:
    ...
    def print_report(self):
        ...
        print(f"Data Source: PDG 2024 (Phys. Rev. D 110, 030001)")
        print("=" * 70)
        ...
        print(f"  Experimental Values (PDG 2024):")
        ...
        print()
        print(f"  Tau mass prediction from Koide:")
        ...
        for angle in ['theta_12', 'theta_13', 'theta_23']:
            data = ckm[angle]
            print(f"  {angle:<10} {data['predicted']:>12.3f}° {data['experimental']:>14.3f}° {data['deviation_percent']:>+11.1f}%")
        ...
        print(f"    Status: {'✓ VALIDATED' if max_ckm_dev < 20 else '⚠ MARGINAL'} (max {max_ckm_dev:.1f}% deviation)")
...
class ExtendedAnalysis:
    ...
    def print_extended_report(self):
        ...
        print("-" * 50)
        print(f"  √(m_gen2/m_gen1) and √(m_gen3/m_gen2) for each sector:")
        print()
        print(f"  {'Sector':<15} {'Gen 1→2':>12} {'Gen 2→3':>12}")
        print(f"  {'-'*15} {'-'*12} {'-'*12}")
        print(f"  {'Leptons':<15} {scaling['leptons']['gen1_to_gen2']:>12.2f} {scaling['leptons']['gen2_to_gen3']:>12.2f}")
        print(f"  {'Up-type':<15} {scaling['up_quarks']['gen1_to_gen2']:>12.2f} {scaling['up_quarks']['gen2_to_gen3']:>12.2f}")
        print(f"  {'Down-type':<15} {scaling['down_quarks']['gen1_to_gen2']:>12.2f} {scaling['down_quarks']['gen2_to_gen3']:>12.2f}")

        # Second generation
        gen2 = self.analyze_charm_generation()
        print("\n[B] SECOND GENERATION STRUCTURE")
        print("-" * 50)
        print(f"  √m ratios (normalized to muon):")
        print(f"    √m_μ  / √m_μ = 1.000")
        ...
        print("-" * 50)
        print(f"  √m ratios (normalized to tau):")
        print(f"    √m_τ  / √m_τ = 1.000")
        ...
        fermions = [
            ('electron', 'MeV', 1), ('muon', 'MeV', 2), ('tau', 'MeV', 3),
            ('up', 'MeV', 1), ('charm', 'MeV', 2), ('top', 'GeV', 3),
            ('down', 'MeV', 1), ('strange', 'MeV', 2), ('bottom', 'MeV', 3),
        ]
