...
def classify_script(name: str, output: str, success: bool) -> str:
    ...
    # Fitted parameters
    fitted_keywords = ["calibrat", "fit", "mcmc", "optimize"]
...
def generate_report(analysis_results: List[ScriptResult], core_results: List[ScriptResult]) -> str:
    ...
    report.append(
        f"  PROVEN (mathematical):  {counts['PROVEN']} ({100*counts['PROVEN']/total:.0f}%)"
    )
    report.append(
        f"  POSTULATED (physical):  {counts['POSTULATED']} ({100*counts['POSTULATED']/total:.0f}%)"
    )
    report.append(
        f"  FITTED (empirical):     {counts['FITTED']} ({100*counts['FITTED']/total:.0f}%)"
    )
    report.append(
        f"  BROKEN (needs fix):     {counts['BROKEN']} ({100*counts['BROKEN']/total:.0f}%)"
    )
