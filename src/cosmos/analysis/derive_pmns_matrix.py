...
def tribimaximal_analysis():
    ...
    print("\n  TBM angles:")
    print(
        f"    sin²θ₁₂ = 1/3 = {1 / 3:.4f} → θ₁₂ = {np.degrees(np.arcsin(s12_TBM)):.2f}°"
    )
    print(
        f"    sin²θ₂₃ = 1/2 = {1 / 2:.4f} → θ₂₃ = {np.degrees(np.arcsin(s23_TBM)):.2f}°"
    )
    print("    sin²θ₁₃ = 0            → θ₁₃ = 0°")
...
def derive_full_pmns():
    ...
    print("  " + "-" * 60)
    print(
        f"  {'Parameter':>12} | {'J₃(O) Prediction':>18} | {'Experiment':>15} | {'Match':>8}"
    )
    print("  " + "-" * 60)
    ...
    for name, pred, exp, unit in params:
        ...
        print(
            f"  {name:>12} | {pred:>15.2f}{unit:>3} | {exp:>12.2f}{unit:>3} | {match:>8}"
        )
